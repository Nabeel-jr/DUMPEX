...
def customer_dashboard(request):
    ...
    noti=Notification.objects.filter(user=request.user).count()
    notification=Notification.objects.filter(user=request.user).order_by('-created_at')[:3]

    return render(request, 'customer/customer_dashboard.html',{'wallet':wallet,'noti':noti,'notification':notification})
