...
def employee_list(request):
    employees = CustomUser.objects.filter(user_type=2,is_verified=0)
...
def delete_employee(request, employee_id):
    employee = get_object_or_404(CustomUser, id=employee_id, user_type=2)  # Ensure user_type 2 (Employee)
    employee.delete()
    messages.success(request, 'Employee deleted successfully.')
    return redirect('manager:manage_employees')
