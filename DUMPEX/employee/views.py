# employee/views.py
from core.models import CustomUser,CollectionSchedule, Notification, SlotBooking
from .forms import UserCreationForm, AddressForm,NotificationForm,CollectionForm
...
def send_notification_to_manager(request):
    ...
    if request.method == 'POST':        
        ...
        rvid= '5'
        Notification.objects.create(user=request.user, message=message,rvid=rvid)
...
def view_manager_notifications(request):
    ...
    notifications = Notification.objects.filter(user=request.user,rvid__isnull=True)  # Adjust the filter as needed
...
def collect(request, booking_id):
    try:
        booking = SlotBooking.objects.get(id=booking_id, customer__is_verified=True,is_confirmed=False)
